      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button className="text-sm border px-4 py-2 rounded-md hover:bg-gray-100 flex items-center gap-1">
              <Icon icon="material-symbols:filter-alt-outline" className="text-lg" />
              Filter
            </button>
        ...
        <div className="bg-white rounded-md shadow-sm divide-y overflow-auto">
          ...
          {stockOpnames.map((item, index) => (
            <div key={index} className="grid grid-cols-12 px-4 py-3 text-sm">
              ...
              <div className="col-span-3 font-semibold">{item.id}</div>
              <div className="col-span-2">{item.productCount}</div>
              <div className="col-span-2">{item.warehouse}</div>
              <div className="col-span-2 text-gray-700">{item.note}</div>
            </div>
          ))}
        </div>
      </div>
